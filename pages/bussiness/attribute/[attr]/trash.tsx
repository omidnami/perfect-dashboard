...
export default function Trash() {
    ...
    useEffect(() => {
        ...
        postData(`attribute/select/0`,{gp:router.query.attr},header)
        }, [router])
    ...
    useEffect(() => {
         ...
         if (response?.total) {
          ...
          if (response?.status) {
            ...
            postData(`attribute/select/0?page=${page}`,null,header)
          }
    ...
    return (
        <main>
            ...
            <Container>
                <Header />
                <h1 style={{marginTop:'25px',marginBottom:'25px',maxWidth:'65%',float:'right'}}> گروه خصوصیات حذف شده</h1>
                <div style={{marginTop:'25px',marginBottom:'25px',maxWidth:'40%',float:'left'}}>
                    <Button onClick={() => router.back()} className="primary"> بازگشت به خصوصیات</Button>
                </div>
